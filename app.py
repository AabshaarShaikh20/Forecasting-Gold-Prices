...
if st.button("Predict Next 30 Days"):
    ...
    st.markdown("<div class='custom-subheader'>Predicted Prices for Next 30 Days</div>", unsafe_allow_html=True)
    ...
    # Plot historical data + predicted data in one graph
    combined_data = pd.concat([historical_data[-30:], prediction_data])
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(combined_data.index, combined_data['price'], label='Price (Last 30 Days & Prediction)', color='blue')
    ax.set_title('Gold Prices: Last 30 Days & Predictions for Next 30 Days')
    ax.set_xlabel('Date')
    ax.set_ylabel('Gold Price (USD)')
    ax.legend()
    st.pyplot(fig)
